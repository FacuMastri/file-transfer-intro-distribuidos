...
class ClientWorker:
    TIMEOUT = 3
    def __init__(self, blockingQueue, clientAddress, fileName, logger):
        self.packet_number = 0
        self.input_queue = blockingQueue
        self.client_address = clientAddress
        self.file = open(f"%s{fileName}" % BUCKET_DIRECTORY, "wb")
        self.logger = logger
        self.socket = socket(AF_INET, SOCK_DGRAM)
        self.socket.sendto(Packet.ack_packet(0), self.client_address) # TODO esto no deberia estar aca
    ...
    def receive_packet(self, packet):
        ...
        try:
            if (packet.finished):
                self.logger.debug(f"Client: {self.client_address} finished. file saved: {packet.filename}")
            ...
            if (packet.packet_number != self.packet_number):
                ...
                self.socket.sendto(Packet.ack_packet(self.packet_number-1), self.client_address)
            ...
            self.socket.sendto(Packet.ack_packet(packet.packet_number), self.client_address)
